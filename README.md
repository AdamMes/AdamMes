# Hi, I'm Adam 👋

AI Engineer · LLM Systems, RAG & Autonomous Agents  

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Adam%20Meshulam-blue?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/adammeshulam) [![Email](https://img.shields.io/badge/Email-adammes23%40gmail.com-important?logo=gmail&logoColor=white)](mailto:adammes23@gmail.com) [![Topics](https://img.shields.io/badge/Topics-AI%20%7C%20LLM%20%7C%20RAG%20%7C%20LangChain-lightgrey)](#)

I design and deploy production-ready GenAI applications—LLM orchestration, RAG pipelines, and autonomous agents—bridging research and reliable infrastructure.

Quick highlights
- ✅ 5 years web development experience.
- ⚙️ Production LLM orchestration (OpenAI, Claude, Gemini, Ollama).
- 🧭 RAG & vector search (FAISS), LangChain, multi-agent systems.
- ☁️ Infra & automation: Python (FastAPI), AWS Bedrock, Lambda, S3, MCP servers.
- 🔁 CI/CD and workflow automation for AI-native products.

## Tech stack
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white&style=flat-square) ![Sass](https://img.shields.io/badge/Sass-CC6699?logo=sass&logoColor=white&style=flat-square) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black&style=flat-square) ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white&style=flat-square) ![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white&style=flat-square) ![MCP](https://img.shields.io/badge/MCP-Model%20Context%20Protocol-1f425f?style=flat-square) ![RAG](https://img.shields.io/badge/RAG-Retrieval--Augmented%20Generation-0D1117?style=flat-square) ![LLMs](https://img.shields.io/badge/LLMs-Claude%20%7C%20Gemini%20%7C%20OpenAI-0D1117?style=flat-square) ![Vibe coding](https://img.shields.io/badge/Vibe%20coding-Claude%20Code%20%7C%20Cursor%20%7C%20Codex-0066CC?style=flat-square) ![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat-square) ![HuggingFace](https://img.shields.io/badge/HuggingFace-FF6C37?style=flat-square) ![AWS](https://img.shields.io/badge/AWS-Bedrock%20%7C%20Lambda%20%7C%20EC2%20%7C%20S3%20%7C%20Rekognition%20%7C%20Polly-1f425f?logo=amazonaws&logoColor=white&style=flat-square) ![N8N](https://img.shields.io/badge/n8n-E34F26?logo=n8n&logoColor=white&style=flat-square) ![FastAPI](https://img.shields.io/badge/FastAPI-005571?logo=fastapi&logoColor=white&style=flat-square) ![Flask](https://img.shields.io/badge/Flask-000000?logo=flask&logoColor=white&style=flat-square) ![FAISS](https://img.shields.io/badge/FAISS-0b0b0b?logo=python&style=flat-square)


---

## Education & Certifications
- DevOps Experts — Professional Certificate, AI Engineering (Jan 2026 – Jul 2026)
- HackerU — Professional Certificate, Full Stack Developer / Web Developer (Sep 2019 – Apr 2020)

---

## Featured Projects
- [VoyaNode](https://github.com/AdamMes/voyanode) — State-of-the-art Retrieval-Augmented Generation (RAG) system designed to transform travel documents (PDFs) into an interactive, intelligent travel assistant. Built on AWS Serverless architecture, it leverages high-performance vector search and generative AI to provide factual, context-aware travel advice.


- [N8N Intelligent cloud finance document analyst](https://github.com/AdamMes/n8n-intelligent-cloud-finance-document-analyst) — An automated document ingestion and semantic analysis pipeline. The system monitors a local folder for corporate financial documents (PDF, TXT, and DOCX), extracts text and embedded images via a Python FastAPI microservice, orchestrates the pipeline in n8n, analyzes content with Google Gemini, enriches metadata, and exports results to Google Sheets, Gmail, and local JSON/Markdown files.

---

## GitHub Stats
![GitHub stats](https://github-readme-stats.vercel.app/api?username=AdamMes&show_icons=true&theme=dark)
![Top languages](https://github-readme-stats.vercel.app/api/top-langs/?username=AdamMes&layout=compact&theme=dark)

---

Thanks for visiting — feel free to reach out if you want to collaborate on AI systems, RAG pipelines, or autonomous agents.
